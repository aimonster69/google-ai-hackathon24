...
def base64_to_pil_image(base64_data):
  """
  Converts a base64 encoded PNG image string to a PIL Image object.

  Args:
      base64_data: The base64 encoded PNG image string.

  Returns:
      A PIL Image object representing the PNG image.

  Raises:
      ValueError: If the data is not valid base64 or is not a PNG image.
  """
  if not base64_data:
...
def dynamic_safety_setting(table_description, df):
    """
...
class FileHandler:
    def __init__(self, url):
        """
        Initialize the FileHandler class with a URL.
    ...
    def check_file_type(self):
        """
        Check the file type based on the URL's extension.
        
        Returns:
        - str: 'csv' if the file is a CSV, 'excel' if it's an Excel file.
        ...
        if self.url.endswith('.csv'):
            return 'csv'
        elif self.url.endswith('.xls') or self.url.endswith('.xlsx'):
            return 'excel'
        else:
    ...
    def read_file(self):
        ...
        # Example usage:
        # url = 'https://example.com/example.csv'  # Replace with your URL
        # file_handler = FileHandler(url)
        # df = file_handler.read_file()  # Read the file
...
class GenerateInsights:
    def __init__(self, my_analysis, data, analysis_file, safety_setting, code_transcript) :
        self.my_analysis = my_analysis
        self.data = data
        self.analysis_file = analysis_file.replace(".json", "")
        self.safety_setting = safety_setting
    ...
    def generate_insights(self, table_description):
        ...
        if insight_choice=='Visualization':
            ...
            while count<2:
                try:
                    visualization_prompt = f'''
                        ...
                        5. Always show x axis labels with a rotation of 90 degrees if the number of labels are more than 8 else 60 degrees by all means.
                        6. If the chart can be built using Seaborn, Geopandas or Plotly then use it
                        7. Refer Code trascript: {self.code_transcript} to write an error free code.
                        8. If number of entities/rows representing are more then plot only the first/top 10 rows (and mention it in the graph that you have done it)
            ...
            img = base64_to_pil_image(png_string)
        ...
        if insight_choice=='Text':
            ...
            print(insights)
        return self.code_transcript, insights, html_string, png_string
...
class VyuEngine:
    ...
    def start_engine(self):
        start = time.time()
        # Reading file
        file_handler = FileHandler(self.job.Database[0])
        df = file_handler.read_file()

        # Creating data dictionary
        table_description = self.job.TableDescription[0]
        data_dict = create_data_dictionary(table_description, df)
        # print(data_dict)

        # Dynamic safety setting
        safety_setting = dynamic_safety_setting(table_description, df)
        print()
        ...
        json_list = json.loads(json.dumps(list(data.to_dict().values())))
        print(json_list)
        analysis_file_name = str(pd.Timestamp.now()).replace(' ', '').replace('-', '').replace(':', '').replace('.', '')
        data.to_json(analysis_file_name + '.json', orient="index")

        # Generating Insights
        # with open(f'{analysis_file_name}', 'r') as f:
        #     data = json.load(f)
        gen_insights = GenerateInsights(my_analysis, data, analysis_file_name + '.json', safety_setting, code_transcript)
        code_transcript, image_name, insights = gen_insights.generate_insights(table_description)
        print('Execution Time: (in mins)',(time.time()-start)/60)
        
        
        self.job.output_file_name = json_list
